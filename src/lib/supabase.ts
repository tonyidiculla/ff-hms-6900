 ...
 * Hospital staff are identified through employee_seat_assignment
...
export async function getUserHospitalId(userId: string): Promise<string | null> {
  try {
    ...
    // Get hospital assignment through employee_seat_assignment
...
export async function getStaffEmployeeId(userId: string): Promise<string | null> {
  try {
    ...
    // Get employee_entity_id from employee_seat_assignment
    const { data: assignment, error: assignmentError } = await supabase
      .from('employee_seat_assignment')
