...
export interface CreateRosteringEntryRequest {
  entity_platform_id?: string; // Optional - will default to platform ID if not provided
  entry_type: RosteringEntryType;
  staff_id?: string;
  staff_name?: string;
  staff_email?: string;
...
export interface UpdateRosteringEntryRequest {
  entity_platform_id?: string; // Added for validation purposes
  effective_date?: string;
  expiry_date?: string;
  day_of_week?: number;
  start_time?: string;
...
export interface RosteringQuery {
  ...
  entry_type?: RosteringEntryType; // Single entry type filter (backward compatibility)
  employee_id?: string; // Single employee filter
  date_from?: string;
  date_to?: string;
  start_date?: string; // Alias for date_from (backward compatibility)
  end_date?: string; // Alias for date_to (backward compatibility)
  day_of_week?: number;
  status?: EntryStatus[];
